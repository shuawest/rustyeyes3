...
ssh -t $REMOTE_HOST "cd $REMOTE_DIR && git config user.email 'deploy@bot.local' && git config user.name 'Deploy Bot' && git stash && git pull && source \$HOME/.cargo/env && cargo build --release --bin rusty-eyes && export DISPLAY=:0 && ./run.sh"
