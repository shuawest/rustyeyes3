...
impl FaceDetector {
    ...
    pub fn detect(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<Rect>> {
        ...
        let best_box = FaceDetector::post_process(&self.anchors, scores_data, boxes_data, score_threshold);
    ...
    fn post_process(anchors: &[(f32, f32, f32, f32)], scores_raw: &[f32], boxes_raw: &[f32], threshold: f32) -> Option<Rect> {
