...
use crate::types::{Landmarks, Point3D, Rect, PipelineOutput};
...
impl FaceMeshPipeline {
    pub fn new(model_path: &str, detector_path: &str) -> Result<Self> {
        ...
        if Path::new(model_path).exists() {
            ...
            Ok(Self { mesh_session: Some(mesh_session), detector, start_time: std::time::Instant::now() })
        } else {
            Ok(Self { mesh_session: None, detector: None, start_time: std::time::Instant::now() })
        }
    }
}
    ...
    fn process(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<PipelineOutput>> {
        ...
        */
        ...
        let (crop, offset_x, offset_y, scale_x, scale_y) = if let Some(rect) = roi {
            ...
            if x < 0.0 { x = 0.0; }
            if y < 0.0 { y = 0.0; }
            if x + w > frame.width() as f32 { w = frame.width() as f32 - x; }
            if y + h > frame.height() as f32 { h = frame.height() as f32 - y; }
            
            let crop = image::imageops::crop_imm(frame, x as u32, y as u32, w as u32, h as u32).to_image();
            ...
            (crop, x, y, sx, sy)
             ...
             (frame.clone(), 0.0, 0.0, frame.width() as f32 / 192.0, frame.height() as f32 / 192.0)
        };
