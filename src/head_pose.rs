...
impl HeadPosePipeline {
    pub fn new(model_path: &str, detector_path: &str) -> Result<Self> {
        ...
        if Path::new(model_path).exists() {
            ...
            Ok(Self { session: Some(session), detector, _detector_path: detector_path.to_string() })
        } else {
            Ok(Self { session: None, detector, _detector_path: detector_path.to_string() })
        }
    }
    ...
    fn process(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<PipelineOutput>> {
        ...
        let (crop, _, _, _, _) = if let Some(rect) = roi {
            ...
            let mut h_clean = size;
             ...
             if x_clean < 0.0 { x_clean = 0.0; }
             if y_clean < 0.0 { y_clean = 0.0; }
             if x_clean + w_clean > frame.width() as f32 { w_clean = frame.width() as f32 - x_clean; }
             if y_clean + h_clean > frame.height() as f32 { h_clean = frame.height() as f32 - y_clean; }

            let crop = image::imageops::crop_imm(frame, x_clean as u32, y_clean as u32, w_clean as u32, h_clean as u32).to_image();
