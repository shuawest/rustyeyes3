...
fn main() -> Result<()> {
    ...
    let api = Api::new()?;
    let repo_v1 = api.repo(Repo::new(
        "vikhyatk/moondream1".to_string(),
        RepoType::Model,
    ));
    let repo_v2 = api.repo(Repo::new(
    ...
    for (name, _) in names {
        ...
        if new_name.contains("vision") && new_name.contains("proj_mlp") {
            new_name = new_name.replace(
                "vision_encoder.encoder.model.visual.proj_mlp",
                "vision_encoder.projection.mlp",
            );
        }
