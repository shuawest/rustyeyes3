...
impl CalibrationManager {
    pub fn new(data_dir: &str) -> Result<Self> {
        ...
        let profile = if Path::new(&profile_path).exists() {
            ...
            let profile: CalibrationProfile =
                serde_json::from_reader(file).ok().unwrap_or_default();
    ...
    pub fn save_data_point(
        ...
        x: f32,
        y: f32,
        inference: Option<PipelineOutput>,
    ) -> Result<u64> {
    ...
    pub fn update_point_with_moondream(
        &mut self,
        timestamp: u64,
        result: crate::types::Point3D,
    ) -> Result<()> {
            ...
            .find(|p| p.timestamp == timestamp)
        {
            ...
            println!(
                "Updated Calibration Point {} with Moondream Data",
                timestamp
            );
        } else {
            ...
            if Path::new(&json_path).exists() {
                ...
                serde_json::to_writer_pretty(json_file, &pt)?;
                println!(
                    "Updated Calibration Point {} with Moondream Data (Disk)",
                    timestamp
                );
            }
        }
