...
impl Pipeline for SimulatedGazePipeline {
    ...
    fn process(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<PipelineOutput>> {
        ...
        if let (Some(PipelineOutput::Landmarks(l)), Some(PipelineOutput::HeadPose(y, p, r))) =
            (mesh_out, pose_out)
        {
...
impl PupilGazePipeline {
    ...
    fn detect_pupil_offset(
        ...
        // Safety check
        if x as u32 + crop_size as u32 > frame.width()
            || y as u32 + crop_size as u32 > frame.height()
        {
            return (0.0, 0.0);
        }
...
impl L2CSPipeline {
    pub fn new(model_path: &str, mesh_path: &str, detector_path: &str) -> Result<Self> {
        ...
        let session = if Path::new(model_path).exists() {
            ...
            println!(
                "[L2CS] Model not found at {}. Gaze will be disabled.",
                model_path
            );
            None
        };
...
impl Pipeline for L2CSPipeline {
    ...
    fn process(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<PipelineOutput>> {
        ...
        if let Some(PipelineOutput::Landmarks(l)) = &mesh_out {
            ...
            if sx + sw > frame.width() as f32 {
                sw = frame.width() as f32 - sx;
            }
            if sy + sh > frame.height() as f32 {
                sh = frame.height() as f32 - sy;
            }

            let crop = image::imageops::crop_imm(frame, sx as u32, sy as u32, sw as u32, sh as u32)
                .to_image();
            ...
            let (yaw_deg, pitch_deg) = if output_count == 1 {
                ...
                if out.len() == 2 {
                    ...
                    eprintln!(
                        "L2CS Single Output mode expects 2 values (Yaw, Pitch), got {}",
                        out.len()
                    );
                    return Ok(None);
                }
            ...
            let (y_gained, p_gained) =
                if let (Some(svr_x), Some(svr_y)) = (&self.svr_x, &self.svr_y) {
                    ...
                    let yg = self.params.yaw_curve * y_raw * y_raw
                        + self.params.yaw_gain * y_raw
                        + self.params.yaw_offset;
                    // Invert pitch result to satisfy main.rs subtraction logic
                    let pg = -(self.params.pitch_curve * p_raw * p_raw
                        + self.params.pitch_gain * p_raw
...
impl MobileGazePipeline {
    pub fn new(model_path: &str, mesh_path: &str, detector_path: &str) -> Result<Self> {
        ...
        let session = if Path::new(model_path).exists() {
            ...
            println!(
                "[MobileGaze] Model not found at {}. Gaze will be disabled.",
                model_path
            );
            None
        };
...
impl Pipeline for MobileGazePipeline {
    ...
    fn process(&mut self, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<Option<PipelineOutput>> {
        ...
        if let Some(PipelineOutput::Landmarks(l)) = &mesh_out {
            ...
            let y_gained = self.params.yaw_curve * y_raw * y_raw
                + self.params.yaw_gain * y_raw
                + self.params.yaw_offset;
            let p_gained = -(self.params.pitch_curve * p_raw * p_raw
                + self.params.pitch_gain * p_raw
                + self.params.pitch_offset);
